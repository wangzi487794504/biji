...
- 套接字：与其它通信机制不同的是，可用于不同机器间的互相通信
  ...
  - 线程通信相对简单，因为线程之间共享进程内的内存，一个例子是多个线程可以访问同一个共享变量
    ...
    **Java 中的通信机制**：volatile、等待/通知机制、join 方式、Inheritable,ThreadLocal、MappedByteBuffer
...
* #### join
  ...
  * ### 1. 使用标志位（volatile 变量）
    ...
    通过使用一个 `volatile` 变量来指示线程 T2 是否继续运行。这是最常见和推荐的方法。
  ...
  public class ThreadTerminationExample {
        ...
        public static void main(String[] args) {
          Thread T2 = new Thread(() -> {
                ...
                System.out.println("T2 is stopping.");
            });
          ...
          try {
            ...
            running = false; // 设置标志位为 false，终止 T2
    ...
    如果线程 T2 正在执行一个可中断的操作（例如 `Thread.sleep()` 或 `Object.wait()`），你可以通过调用 `interrupt()` 来中断它。
            ...
            Thread T2 = new Thread(() -> {
                try {
                    while (!Thread.currentThread().isInterrupted()) {
                        System.out.println("T2 is running...");
                        Thread.sleep(1000); // 可中断的操作
                    }
                } catch (InterruptedException e) {
                    Thread.currentThread().interrupt(); // 如果被中断，设置中断状态
                }
                System.out.println("T2 is stopping.");
            });
    ...
    如果你使用 `ExecutorService` 提交任务，可以通过调用 `Future.cancel()` 方法优雅地取消线程。
    ...
    无论采用哪种方法，都要保证线程能够在安全的状态下停止，并且能够妥善处理资源释放、状态保存等任务。使用标志位和 `interrupt()` 方法是最常用和推荐的方式，适用于大多数情况。
