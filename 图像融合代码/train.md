...
from loss_function import SSIM_LOSS, L1_LOSS, Fro_LOSS, _tf_fspecial_gauss,gradient
...
def train(original_imgs_path, validatioin_imgs_path, save_path, model_pre_path, ssim_weight, EPOCHES_set, BATCH_SIZE, IS_Validation, debug=False, logging_period=1):
    if debug:#日志
    ...
    with tf.Graph().as_default(), tf.compat.v1.Session() as sess:
        ...
        g_generated_img = gradient(generated_img)
        ...
        grad_loss = tf.reduce_sum(tf.square(g_original - g_generated_img))
        grad_loss = grad_loss/(BATCH_SIZE*HEIGHT*WIDTH)
        ssim_loss = 1 - ssim_loss_value

        loss = ssim_weight*ssim_loss + pixel_loss + grad_loss
            ...
            for batch in range(n_batches):
                ...
                if debug:
                    ...
                    if is_last_step or step % logging_period == 0:
                        elapsed_time = datetime.now() - start_time
                        _ssim_loss, _loss, _p_loss, _g_loss  = sess.run([ssim_loss, loss, pixel_loss ,grad_loss], feed_dict={original: original_batch})
                        Loss_all[count_loss] = _loss
                        Loss_ssim[count_loss] = _ssim_loss
                        Loss_pixel[count_loss] = _p_loss
                        Loss_grad[count_loss] = _g_loss
                        print('epoch: %d/%d, step: %d,  total loss: %s, elapsed time: %s' % (epoch, EPOCHS, step, _loss, elapsed_time))
                        print('p_loss: %s, g_loss: %s, ssim_loss: %s ,w_ssim_loss: %s ' % (_p_loss, _g_loss,_ssim_loss, ssim_weight * _ssim_loss))
                        ...
                        if IS_Validation:
                            ...
                            for v in range(val_batches):
                                ...
                                val_ssim, val_pixel,val_grad = sess.run([ssim_loss, pixel_loss,grad_loss], feed_dict={original: val_original_batch})
                            ...
                            print('validation value, SSIM: %s, Pixel: %s, Grad: %s, elapsed time: %s' % (val_ssim_acc/val_batches, val_pixel_acc / val_batches,val_grad_acc / val_batches, val_es_time))
                            print('------------------------------------------------------------------------------')
                        count_loss += 1
        ...
        scio.savemat('./models/loss/DeepDenseLossGradData.mat'+str(ssim_weight)+'', {'loss_grad': loss_grad_data})
        ...
        if IS_Validation:
            ...
            validation_grad_data = Val_grad_data[:count_loss]
            scio.savemat('./models/val/Validation_pixel_Data.mat' + str(ssim_weight) + '', {'val_grad': validation_grad_data})
